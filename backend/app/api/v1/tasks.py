...
def create_task(payload: TaskCreate, service: TaskServiceDependency) -> AgentTask:
    try:
...
def list_tasks(
    ...
    tasks = service.list(limit, task_status)
    return AgentTaskList(items=tasks, total=len(tasks))


@router.get("/{task_id}", response_model=AgentTask)
def get_task(task_id: str, service: TaskServiceDependency) -> AgentTask:
    try:
...
def task_events(task_id: str, service: TaskServiceDependency) -> StreamingResponse:
    try:
...
def cancel_task(task_id: str, service: TaskServiceDependency) -> AgentTask:
    try:
